...
// Metode za gradnju stabla izraza

// Stvara novi samostalni cvor (nije spojen na stablo)
node CreateNode(nodetype x) {
     node p = new btr;
     (*p).label = x;
...
// Postavlja postojeci cvor kao lijevo dijete
void AttachLeft(node parent, node child) {
     if (parent == lambda) return;
     (*parent).left = child;
     if (child != lambda)
         (*child).parent = parent;
}

// Postavlja postojeci cvor kao desno dijete
